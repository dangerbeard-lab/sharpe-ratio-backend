...
def home():
    return jsonify({
        ...
        "endpoints": {
            ...
            "/api/portfolio": "Get all portfolio prices"
        }
    })
...
def get_bitcoin_price():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'data' in data and 'BTC' in data['data']:
                ...
                return jsonify({"price": price, "source": "CoinDesk"})
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({"price": price, "source": "CoinGecko"})
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'Realtime Currency Exchange Rate' in data:
                ...
                return jsonify({"price": price, "source": "Alpha Vantage"})
    except Exception as e:
        logger.error(f"Alpha Vantage crypto error: {e}")
    
    # Ultimate fallback
    return jsonify({"price": 170000, "source": "fallback"})
...
def get_stock_price(symbol):
    ...
    return jsonify({"symbol": symbol, "price": price, "source": "fallback"})
...
def get_fx_rate_internal(from_currency, to_currency):
    """Internal function to get FX rates"""
    
    # Check cache
    cache_key = f"fx_{from_currency}_{to_currency}"
    if cache_key in cache:
        cached_time, cached_rate = cache[cache_key]
        if (datetime.now() - cached_time).seconds < cache_timeout * 10:  # Longer cache for FX
            return cached_rate
    ...
    return default_rates.get(key, 1.5)

@app.route('/api/portfolio', methods=['POST'])
def get_portfolio_prices():
    """Get all portfolio prices in one call"""
    ...
    results['SPY'] = spy_data['price'] * fx_rates['USD']
